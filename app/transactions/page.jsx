          ...
          <div className="pt-3 border-t border-blush">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div>
                ...
                <input type="datetime-local" value={filters.from} onChange={e => set('from', e.target.value)} className="w-full px-3 py-2 rounded-xl border border-blush bg-white text-sm focus:border-primary outline-none" />
              </div>
              <div>
                <label className="text-[11px] text-muted font-medium uppercase tracking-wide mb-1 block">Sampai</label>
                <input type="datetime-local" value={filters.to} onChange={e => set('to', e.target.value)} className="w-full px-3 py-2 rounded-xl border border-blush bg-white text-sm focus:border-primary outline-none" />
              </div>
        ...
        {loading ? <div className="p-6 text-center text-muted text-sm animate-pulse">Memuat…</div> : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.items.map((t, i) => (
                  <tr key={t.id} className={`border-b border-blush/40 hover:bg-blush/30 transition ${i % 2 === 1 ? 'bg-cream/20' : ''}`}>
...
function PayModal({ appts, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm grid place-items-center z-50 p-4" onClick={onClose}>
      <form onClick={e => e.stopPropagation()} onSubmit={submit} className="bg-white rounded-2xl shadow-soft p-6 w-full max-w-lg space-y-4 border border-blush">
        ...
        <div className="text-sm flex justify-between bg-blush rounded-xl px-4 py-3"><span className="text-muted">Total</span><span className="font-bold text-ink">{formatRupiah(total)}</span></div>
        {err && <p className="text-sm text-rose-600 bg-rose-50 px-3 py-2 rounded-xl">{err}</p>}
        <div className="flex gap-2 justify-end">
          <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl text-sm text-muted hover:bg-cream transition">Batal</button>
...
function QrisModal({ txId, total, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm grid place-items-center z-50 p-4" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-soft p-6 w-full max-w-md space-y-4 text-center border border-blush">
        ...
        {state.phase === 'ready' && !paid && <>
          <div className="flex justify-center"><img src={state.qrUrl} alt="QRIS" className="w-56 h-56 border-2 border-blush rounded-2xl" /></div>

...
function Modal({initial,onClose,onSaved}){
 ...
 return <div className="fixed inset-0 bg-black/40 grid place-items-center z-50 p-4" onClick={onClose}><form onClick={e=>e.stopPropagation()} onSubmit={submit} className="bg-card rounded-lg p-6 w-full max-w-lg space-y-3"><h3 className="font-semibold text-lg">{initial?'Edit':'Tambah'} Terapis</h3><div><label className="text-xs text-muted">Nama</label><Input required value={form.name} onChange={e=>set('name',e.target.value)}/></div><div><label className="text-xs text-muted">Phone</label><Input value={form.phone} onChange={e=>set('phone',e.target.value)}/></div><div><label className="text-xs text-muted">Spesialisasi</label><Input value={form.specialization} onChange={e=>set('specialization',e.target.value)}/></div><div><label className="text-xs text-muted">Bio</label><Textarea rows={2} value={form.bio} onChange={e=>set('bio',e.target.value)}/></div><label className="flex gap-2 text-sm"><input type="checkbox" checked={form.isActive} onChange={e=>set('isActive',e.target.checked)}/>Aktif</label>{err&&<p className="text-sm text-rose-600">{err}</p>}<div className="flex justify-between"><Btn>Simpan</Btn>{initial&&<button type="button" onClick={del} className="px-4 py-2 rounded-md bg-rose-100 text-rose-700 text-sm">Hapus</button>}</div></form></div>
}
